...
function takeWhileInternal(sequence, values, predicate, callback) {
    sequence.head(function (err, option) {
...
function ObjectIterator(obj) {
    ...
    this.next = function (callback) {
        arrayIterator.next(function (err, value) {
...
stream(fs.ReadStream("./test/badfilename", {encoding:"utf8"})).realise(function (err, data) {
    console.log("Data: " + data[0]);
    console.log("Data: " + data.length);
//            test.ok(err instanceof Error);
//            test.ok(data, ["Sample File Data"]);
//    test.done();
});